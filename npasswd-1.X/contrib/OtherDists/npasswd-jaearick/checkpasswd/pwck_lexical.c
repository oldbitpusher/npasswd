...
char	*mesg;		/* Message buffer */
{
	...
	for (p = password; *p; p++) {
		/* check for run of same characters */
		if (*p != last) {
			last = *p;
			rc = 1;
		}
		...
		/* checks for character in illegal character list */
		/* (jae): moved this outside the unprintable char */
		/* list check because we wanted to exclude some   */
		/* "printable" characters, ie blank and tab.      */
		if (index(illegalcc, *p)) {
#ifdef DEBUG
			printf("checking illegal characters\n");
#endif
			(void) sprintf(mesg,
		...
		/* checks for unprintable characters, assuming ascii */
		if (*p < ' ' || *p > '~') {
			if (print_only) {
				(void) strcpy(mesg,
