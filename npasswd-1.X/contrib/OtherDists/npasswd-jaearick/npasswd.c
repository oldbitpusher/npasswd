 ...
 */
#ifdef SUNOS4
#define WAITPID		/* use waitpid() instead of wait() */
#endif
...
#ifndef lint
static char sccsid[] = "@(#)npasswd.c	1.18 12/20/91 (cc.utexas.edu) /tmp_mnt/usr/share/src/private/ut/share/bin/passwd/SCCS/s.npasswd.c";
#endif
...
#ifdef	sun
static char	*options = "alyd:e:F:n:fPsRVx:";/* Command line options */
#else
static char	*options = "fPRsV";	/* Command line options */
#endif
static int	retries = 3;		/* Retry limit */
static int	from_prog = 0;		/* Data source is a program */
static int	need_oldpw = 0;		/* Require old passwd */
...
char	*argv[];
{
	...
	while ((opt = getopt(argc, argv, options)) != EOF) {
		...
		case 'R':		/* Require old passwd even for su */
			if (getuid())
				quit(0, "Option \"-R\" reserved for super-user.\n");
			need_oldpw = 1;
			break;
		case 'V':
 ...
 *	previous version used a system() call, eliminated here
 *	for security reasons.  Complain if file not found, or
 *	just dump it to stdout otherwise.
 */
motd(fn, complaint)
